...
void setup_scr_alarm(lv_ui *ui){
	...
	//Write codes alarm_btn_back_a
	ui->alarm_btn_back_a = lv_btn_create(ui->alarm, NULL);

	//Write style LV_BTN_PART_MAIN for alarm_btn_back_a
	static lv_style_t style_alarm_btn_back_a_main;
	lv_style_reset(&style_alarm_btn_back_a_main);

	//Write style state: LV_STATE_DEFAULT for style_alarm_btn_back_a_main
	lv_style_set_radius(&style_alarm_btn_back_a_main, LV_STATE_DEFAULT, 50);
	lv_style_set_bg_color(&style_alarm_btn_back_a_main, LV_STATE_DEFAULT, lv_color_make(0xff, 0xff, 0xff));
	lv_style_set_bg_grad_color(&style_alarm_btn_back_a_main, LV_STATE_DEFAULT, lv_color_make(0xff, 0xff, 0xff));
	lv_style_set_bg_grad_dir(&style_alarm_btn_back_a_main, LV_STATE_DEFAULT, LV_GRAD_DIR_VER);
	lv_style_set_bg_opa(&style_alarm_btn_back_a_main, LV_STATE_DEFAULT, 255);
	lv_style_set_border_color(&style_alarm_btn_back_a_main, LV_STATE_DEFAULT, lv_color_make(0x01, 0xa2, 0xb1));
	lv_style_set_border_width(&style_alarm_btn_back_a_main, LV_STATE_DEFAULT, 2);
	lv_style_set_border_opa(&style_alarm_btn_back_a_main, LV_STATE_DEFAULT, 255);
	lv_style_set_outline_color(&style_alarm_btn_back_a_main, LV_STATE_DEFAULT, lv_color_make(0xd4, 0xd7, 0xd9));
	lv_style_set_outline_opa(&style_alarm_btn_back_a_main, LV_STATE_DEFAULT, 255);
	lv_obj_add_style(ui->alarm_btn_back_a, LV_BTN_PART_MAIN, &style_alarm_btn_back_a_main);
	lv_obj_set_pos(ui->alarm_btn_back_a, 243, 199);
	lv_obj_set_size(ui->alarm_btn_back_a, 70, 35);
	ui->alarm_btn_back_a_label = lv_label_create(ui->alarm_btn_back_a, NULL);
	lv_label_set_text(ui->alarm_btn_back_a_label, "返回");
	lv_obj_set_style_local_text_color(ui->alarm_btn_back_a_label, LV_LABEL_PART_MAIN, LV_STATE_DEFAULT, lv_color_make(0x00, 0x00, 0x00));
	lv_obj_set_style_local_text_font(ui->alarm_btn_back_a_label, LV_LABEL_PART_MAIN, LV_STATE_DEFAULT, &lv_font_simsun_12);
}